...
import (
	...
	"golang-gocb-couchbase/internal/folksdev-fiber-rest-api/application/controller/request"
	"golang-gocb-couchbase/internal/folksdev-fiber-rest-api/application/controller/response"
...
func NewUserController(userQueryService query.IUserQueryService,
	userCommandHandler user.ICommandHandler,
	customValidator validation.ICustomValidator) IUserController {
...
func (c *userController) Save(ctx *fiber.Ctx) error {
	...
	if validationErrors := c.customValidator.Validate(req); validationErrors != nil {
		fmt.Printf("userController.Save INVALID request: %#v\n - ERROR: %#v", req, validationErrors)
		return ctx.Status(http.StatusBadRequest).JSON(validationErrors)
	}
